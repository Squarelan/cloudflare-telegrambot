...
4. **Configure Environment Variables**
   ...
   **Optional Variables:**
   ...
   - `ENV_WELCOME_MESSAGE`: Welcome message, customize as needed, default is "Welcome to the bot"
...
3. **Topic Management** (alternative block/unblock, more convenient, no typing needed)
   ...
   - Delete topic: After deleting a topic, the user only needs to send one additional message to recreate the topic, similar to `/clear`

PS: If you want to permanently block and remove (clear) a conversation, use the same type of commands, such as `/block` followed by `/clear`, or close the conversation and then delete it.
